...
export const Home = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scaleValue = Math.min(0.5 + (yScroll / maxScroll) * 0.2, 1); // Rapid scaling to 100%
      const shiftValue = Math.min((yScroll / maxScroll) * 20, 20); // Faster shift outwards
  ...
  const calcTranslate = function (shift) {
    ...
    return scrollY >= 250 ? -((scrollY / 25) * shift) : -(shift * 3);
  };
